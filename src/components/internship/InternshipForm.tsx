...
export const InternshipForm = () => {
  ...
  return (
    <div className="bg-zinc-900/90 border border-zinc-800 p-3 sm:p-6 md:p-8 rounded-xl shadow-xl mb-6 md:mb-12 w-full max-w-4xl mx-auto my-6 sm:my-12">
      {/* Progress bar */}
      <div className="mb-5 md:mb-10 px-1 sm:px-4">
        ...
        <div className="relative mb-5 md:mb-8">
            ...
            {STEPS.map((s) => {
              ...
              return (
                ...
                >
                  <div 
                    className={`
                      relative z-10 flex items-center justify-center w-5 h-5 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full text-[10px] sm:text-xs md:text-sm font-medium transition-all duration-300
                      ${status === 'current' ? 'bg-red-500 text-white' : 
                        status === 'completed' ? 'bg-green-500 text-white' : 'bg-zinc-800 text-zinc-400'}
                    `}
                  >
                    {status === 'completed' ? (
                      <svg className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <span 
                    className={`
                      mt-1 sm:mt-2 text-[8px] sm:text-[10px] md:text-xs font-medium transition-colors duration-300 max-w-[60px] text-center
          ...
          {/* Connecting line */}
          <div className="absolute top-2.5 sm:top-3.5 md:top-4 left-0 right-0 h-0.5 -translate-y-1/2 bg-zinc-800">
            <div 
